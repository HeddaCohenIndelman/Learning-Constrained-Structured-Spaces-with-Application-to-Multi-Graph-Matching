...
class PascalVOC:
    ...
    def get_pair(self, cls=None, shuffle=True, tgt_outlier=False, src_outlier=False):
        ...
        '''
        with open('imbalance.txt','a') as im_file:
            rows=perm_mat.shape[0]
            cols=perm_mat.shape[1]
            im_file.write(str(min(rows,cols))+'\t'+ str(max(cols,rows))+'\t' + anno_dict['cls']+'\n' )
        '''
        row_list = []
        col_list = []
    ...
    def get_pair_unbalanced(self, cls=None, shuffle=True):
        """
        Randomly get a pair of objects from VOC-Berkeley keypoints dataset
        :param cls: None for random class, or specify for a certain set
        :param shuffle: random shuffle the keypoints
        ...
        effective_src_kpt_len = min(cfg.source_partial_kpt_len, len(anno_pair[0]['keypoints']))
        effective_trg_kpt_len = min(cfg.target_partial_kpt_len, len(anno_pair[1]['keypoints']))

        row_list = []
        perm_mat = np.zeros(shape=(effective_src_kpt_len, effective_trg_kpt_len), dtype=np.float32)

        #randomnly filter only (effective) n1 from the first and (effective) n2 from the second
        anno_pair[0]['keypoints'] = random.sample(anno_pair[0]['keypoints'], effective_src_kpt_len)
        anno_pair[1]['keypoints'] = random.sample(anno_pair[1]['keypoints'], effective_trg_kpt_len)
    ...
    def get_multi(self, cls=None, num=2, shuffle=True, filter_outlier=True):
        ...
        if filter_outlier:
            ...
            perm_mat = [np.zeros([len(anno_list[x]['keypoints']), len(anno_list[y]['keypoints'])], dtype=np.float32) for x in range(len(anno_list)) for y in range(x+1,len(anno_list))]
            k = -1
            for x in range(len(anno_list)):
                for y in range(x+1,len(anno_list)):
                    k += 1
                    for i, keypoint in enumerate(anno_list[x]['keypoints']):
                        for j, _keypoint in enumerate(anno_list[y]['keypoints']):
                            if keypoint['name'] == _keypoint['name']:
                                perm_mat[k][i, j] = 1
                                break
